...
<head>
    ...
    <style>
        ...
        .badge { display: inline-block; background: #FFF3E8; color: #FF6600; padding: 0.35rem 0.75rem; border-radius: 999px; font-size: 0.85rem; font-weight: 700; margin-bottom: 1rem; }
        .event-title { font-size: 1.1rem; font-weight: 700; margin: 0.5rem 0; }
...
<body>
    <div class="wrapper">
        ...
        <div class="body">
            ...
            <span class="badge">{{ $reminderType->label() }}</span>
            <p>L'événement auquel vous êtes inscrit approche :</p>
            <p class="event-title">{{ $event->title }}</p>
            <p class="meta">
                {{ $event->start_date->translatedFormat('l j F Y · H:i') }}
                — {{ $event->end_date->format('H:i') }}<br>
                @if($event->location)
                    {{ $event->location }}
            ...
            <a href="{{ route('events.show', ['event' => $event->resolveSlug()]) }}" class="btn">
                Voir l'événement
            </a>
        </div>
        <div class="footer">
