...
<head>
  ...
  <style>
    ...
    .excerpt { color:#555; line-height:1.7; margin:0 0 20px; }
    .info-box { background:#f8f9fa; border-left:4px solid #e8590c; border-radius:0 8px 8px 0; padding:16px 20px; margin-bottom:28px; }
...
<body>
  <div class="wrapper">
    ...
    <div class="body">
      ...
      <div class="excerpt">{{ Str::limit(strip_tags($event->description ?? ''), 200) }}</div>

      <div class="info-box">
        <p><strong>📅 Date :</strong> {{ $event->starts_at->format('d/m/Y à H:i') }}</p>
        <p><strong>📍 Lieu :</strong> {{ $event->location ?? 'En ligne' }}</p>
        @if($event->online_url)
          <p><strong>🔗 Lien :</strong> {{ $event->online_url }}</p>
        @endif
      </div>

      <a href="{{ route('events.show', $event->slug) }}" class="btn">S'inscrire à l'événement →</a>
    </div>
    <div class="footer">
