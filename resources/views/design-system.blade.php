...
@php
/* ── Snippets (view-level, pas de données métier) ──────── */
$snippets = [
    'stat'         => '<x-card.stat icon="group" value="500+" label="Membres Actifs" />',
    'stat2'        => '<x-card.stat icon="forum" value="1.8k" label="Questions posées" />',
    'stat3'        => '<x-card.stat icon="event" value="48" label="Événements organisés" />',
    'stat_sec'     => '<x-card.stat-secondary icon="forum" value="1.2k" description="Messages mensuels sur le forum" />',
    'stat_sec2'    => '<x-card.stat-secondary icon="work" value="24" description="Offres d\'emploi actives" />',
    'event'        => '<x-card.event :event="$event" />',
    'post'         => '<x-card.post :post="$post" />',
    'thread'       => '<x-card.forum-thread :thread="$thread" />',
    'job'          => '<x-card.job :job="$job" />',
    'member_gold'  => '<x-card.member :member="$member" :rank="1" />',
    'member_silv'  => '<x-card.member :member="$member" :rank="2" />',
    'member_plain' => '<x-card.member :member="$member" />',
];
...
<div class="min-h-screen bg-gray-50 text-gray-900 font-[Nunito]">
    ...
    {{-- ── Top bar ──────────────────────────────────────────── --}}
    <header class="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-gray-200 px-6 py-4 flex items-center gap-3">
        <span class="material-icons text-[#E3342F]">palette</span>
        <div>
    ...
    <div class="flex">
        ...
        {{-- ── Sidebar ──────────────────────────────────────── --}}
        <aside class="sticky top-[61px] h-[calc(100vh-61px)] w-56 shrink-0 bg-white border-r border-gray-200 overflow-y-auto p-5 hidden lg:block">
        ...
        <main class="flex-1 min-w-0 p-6 lg:p-10 space-y-16">
            ...
            <section id="stats" data-section>
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Métriques clés homepage. Icône + grand chiffre + label.</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    @foreach([
                        ['key' => 'stat',  'icon' => 'group', 'value' => '500+', 'label' => 'Membres Actifs'],
                        ['key' => 'stat2', 'icon' => 'forum', 'value' => '1.8k', 'label' => 'Questions posées'],
                        ['key' => 'stat3', 'icon' => 'event', 'value' => '48',   'label' => 'Événements organisés'],
                    ] as $s)
                        <div class="rounded-xl overflow-hidden border border-gray-200 bg-white">
                            <div class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-200">
                                ...
                                @include('design-system._copy-btn', ['snippet' => $snippets[$s['key']]])
                            </div>
                            <div class="p-4">
                                <x-card.stat :icon="$s['icon']" :value="$s['value']" :label="$s['label']" />
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <section id="stats-secondary" data-section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                    @foreach([
                        ['key' => 'stat_sec',  'icon' => 'forum', 'value' => '1.2k', 'desc' => 'Messages mensuels sur le forum'],
                        ['key' => 'stat_sec2', 'icon' => 'work',  'value' => '24',   'desc' => "Offres d'emploi actives"],
                    ] as $s)
                        <div class="rounded-xl overflow-hidden border border-gray-200 bg-white">
                            <div class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-200">
                            ...
                            <div class="p-4">
                                <x-card.stat-secondary :icon="$s['icon']" :value="$s['value']" :description="$s['desc']" />
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <section id="members" data-section>
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Avatar (fallback initiales), pseudo, points, badges rang 🥇🥈🥉.</p>
                </div>

                <div class="max-w-sm flex flex-col gap-4">
                    @foreach([
                        ['key' => 'member_gold',  'rank' => 1, 'points' => 1240],
                        ['key' => 'member_silv',  'rank' => 2, 'points' => 980],
                        ['key' => 'member_plain', 'rank' => null, 'points' => 640],
                    ] as $m)
                        @php $m_data = array_merge($member, ['points' => $m['points']]); @endphp
                        <div class="rounded-xl overflow-hidden border border-gray-200 bg-white">
                            <div class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-200">
                                <span class="text-xs font-mono text-gray-400">x-card.member</span>
                                @include('design-system._copy-btn', ['snippet' => $snippets[$m['key']]])
                            </div>
                            <div class="p-4">
                                <x-card.member :member="$m_data" :rank="$m['rank']" />
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    const observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            ...
            navLinks.forEach(link => {
                ...
                link.classList.toggle('text-[#E3342F]', active);
                link.classList.toggle('bg-red-50', active);
